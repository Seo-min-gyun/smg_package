...
//    private var months = [String]()
    ...
    open var maximumDate: Date {
        ...
        get {
            return _maximumDate ?? formattedDate(from: calendar.date(byAdding: .year, value: 80, to: Date()) ?? Date())
        }
    }
...
//            return _minimumDate ?? formattedDate(from: Date())
            return _minimumDate ?? formattedDate(from: calendar.date(byAdding: .year, value: -80, to: Date()) ?? Date())
        }
    }
...
//            }
//            self.selectRow(month - 1, inComponent: 1, animated: animated) //자동선택
            if let firstYearIndex = self.years.firstIndex(of: year) {
